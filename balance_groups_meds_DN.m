function [matched_indices, chi_stat_all] = balance_groups_meds_DN(metadata, data_meds, drug_naive, nmatch, nmatch2)

%==========================================================================
% BALANCE_GROUPS_MEDS FUNCTION - version for drug-naive comparison
%
% Creates matched participant indices for medication-group comparisons with drug-naive subjects.
%
...
%
%   drug_naive : binary labels defining which patients can be classified as
%                drug-naive (do not take any psychotropic drugs).
%
% OUTPUT
...
for l = 1:size(data_meds, 2)
    ...
    if sum(data_meds(:, l)) > 0
    
        % Identify participants who take this medication class.
        idx_med = find(data_meds(:, l) == 1);
        % Identify drug-naive participants.
        idx_nonmed = find(drug_naive == 1);
    ...
    % Preallocate chi-square statistics for this medication class.
        ...
        for match_rep = 1:nmatch
            ...
            n_attempts_best = 1;
            previous_matches = unique(matched_indices{l}{match_rep});
            ...
            if numel(selected_controls) ~= size(match_descriptors_med, 1)
                warning(['The number of selected controls does not match ', ...
                         ...
                         'class %d, repetition %d.'], l, match_rep);
            end
            ...
            if groups_reversed
                matched_indices{l}{match_rep} = [idx_nonmed(selected_controls), idx_med];
            else
                matched_indices{l}{match_rep} = [idx_med, idx_nonmed(selected_controls)];
            end
        end
